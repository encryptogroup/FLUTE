...
for(int i=offset+(1<<29)-1-268435456;i>=offset+(1<<29)-503316480;--i)poly[i]^=poly[i+268435456];
...
for(int i=offset+(1<<28)-1-251658240;i>=offset+(1<<28)-267386880;--i)poly[i]^=poly[i+251658240];
...
for(int i=offset+(1<<27)-1-67108864;i>=offset+(1<<27)-100663296;--i)poly[i]^=poly[i+67108864];
...
for(int i=offset+(1<<26)-1-50331648;i>=offset+(1<<26)-66846720;--i)poly[i]^=poly[i+50331648];
...
for(int i=offset+(1<<25)-1-16777216;i>=offset+(1<<25)-33423360;--i)poly[i]^=poly[i+16777216];
...
for(int i=offset+(1<<24)-1-16711680;i>=offset+(1<<24)-16776960;--i)poly[i]^=poly[i+16711680];
...
for(int i=offset+(1<<23)-1-4194304;i>=offset+(1<<23)-6291456;--i)poly[i]^=poly[i+4194304];
...
for(int i=offset+(1<<22)-1-3145728;i>=offset+(1<<22)-3932160;--i)poly[i]^=poly[i+3145728];
...
for(int i=offset+(1<<21)-1-1048576;i>=offset+(1<<21)-1966080;--i)poly[i]^=poly[i+1048576];
...
for(int i=offset+(1<<20)-1-983040;i>=offset+(1<<20)-1048560;--i)poly[i]^=poly[i+983040];
...
for(int i=offset+(1<<19)-1-262144;i>=offset+(1<<19)-393216;--i)poly[i]^=poly[i+262144];
